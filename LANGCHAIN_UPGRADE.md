...
| Характеристика       | Старая версия              | Новая версия (Langchain)    |
| -------------------- | -------------------------- | --------------------------- |
| **Архитектура**      | Кастомная реализация       | Langchain концепции         |
| **Системный промпт** | Короткий (2 строки)        | Детальный (50+ строк)       |
| **SQL Security**     | Мягкая проверка в описании | Жесткая проверка в коде     |
...
| **Структура БД**     | Не описана                 | Полностью задокументирована |
...
### 2. Усиленная безопасность SQL

**Было:**

- Упоминание в описании функции
- Проверка только на уровне промпта
...
- Жесткая проверка в коде перед выполнением
